    ...
    hover:bg-white-500/60 p-2 hover:shadow-lg transition-all ease-in-out">
      ...
      <div className='w-full flex gap-2 flex-wrap'>
        {education?.technologies.map((tech, index) => (
          <p key={index} className="font-texturina text-[0.625rem] font-medium pb-2 text-center text-black-400 bg-blue-200/25 border border-white-100 rounded px-2 py-1">{tech}</p>
        ))}
      </div>
    </div>
  )
